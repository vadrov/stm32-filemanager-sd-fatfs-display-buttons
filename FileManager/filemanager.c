...
#include "string.h"
#include "fatfs.h"    	//библиотека FATFS
...
static uint8_t findext(char *name, const char *str_types)
{
	...
	if (!f || j == 1) return 0;
	if (strstr(str_types, &name[j - 1])) return 1;
	return 0;
}
...
static void DrawIcon(LCD_Handler *lcd, int x, int y, const Icon_Image *icon, int w, int h)
{
	//выделяем память под масштабированный значок
#ifdef LCD_DYNAMIC_MEM
	uint16_t *data = (uint16_t *)malloc(2 * w * h);
#else
	...
	for (i = 0; i < h; i++) {
		...
		for (j = 0; j < w; j++) {
			j1 = (icon->width * j) / w;
			col16 = *(icon->data + i1 * icon->width + j1);
			if (lcd->data_bus == LCD_DATA_8BIT_BUS) col16 = (col16 >> 8) | ((col16 & 0xFF) << 8);
			*p_data++ = col16;
		}
	}
	//если включено DMA, то используем его для вывода значка на дисплей
	if (lcd->spi_data.dma_tx.dma)
	{
		...
		lcd->tmp_buf = data;
#endif
		LCD_DrawImage(lcd, x, y, w, h, data, 1);
	}
	//DMA не включено - выводим значок на дисплей без DMA
	else
	{
		LCD_DrawImage(lcd, x, y, w, h, data, 0);
#ifdef LCD_DYNAMIC_MEM
		free(data);
#endif
	}
