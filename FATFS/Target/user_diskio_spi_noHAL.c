 ...
 ******************************************************************************
  ...
  * Portions copyright (C) 2019, VadRov, all rights reserved.
  *
  ...
  ******************************************************************************
 *
 *  низкоуровневый драйвер FatFs для работы с SD картой по SPI, в том числе, с DMA
 *  Доработано: 2019, VadRov
 *  версия 1.1 (NO_HAL, без HAL, частично LL)
 *
...
{
	static uint8_t tmp = 0xFF; //важно указать для переменной static
	const uint8_t shift[8] = {0, 6, 16, 22, 0, 6, 16, 22}; //битовое смещение во флаговых регистрах IFCR (L и H) и ISR(L и H)
	...
	while ((dma_RX->CR & DMA_SxCR_EN) || (dma_TX->CR & DMA_SxCR_EN)) {__NOP();} //ждем отключения потоков DMA
	...
	*ifcr_tx = 0x3F<<shift[SD_DMA_STREAM_TX];
	//разрешаем spi принимать запросы от потоков DMA
	SPI_SD->CR2 |= (SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN); //установкой/сбросом этих битов контрольного регистра, мы разрешаем/запрещаем dma управлять spi
														//RXDMAEN - разрешает/запрещает запросы от DMA на прием данных
														//TXDMAEN - разрешает/запрещает запросы от DMA на передачу данных
	...
	dma_RX->FCR &= ~DMA_SxFCR_FEIE;																	//запрещает/разрешает прерывания по
																									...
																									//FE - прерывание по ошибке FIFO (обработка очереди данных)
	...
	while (!(((*isr_tx) & (DMA_LISR_TCIF0<<shift[SD_DMA_STREAM_TX])) && ((*isr_rx) & (DMA_LISR_TCIF0<<shift[SD_DMA_STREAM_RX])))) {__NOP();}
